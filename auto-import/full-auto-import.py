...
def click_file(clickType="double", file_path="", center=False, duration=0, add_left=0, add_top=0):
    """Click the file on the screen at the given file path."""
...
def find_file_position(file_path, center=False):
    """Find the position of the file on the screen at the given file path."""
    ...
    for attempt in range(max_attempts):
        ...
        if file_location is None and os.path.exists(f"imagelocate/{os.path.basename(filename)}2{file_extension}"):
            print(f'Secondary File check: imagelocate/{os.path.basename(filename)}2{file_extension}')
            img2 = cv2.imread(f"imagelocate/{os.path.basename(filename)}2{file_extension}")
...
def detect_assetbundle_type(filename):
    print(f"[DETECTING ASSETBUNDLE TYPE] Filename: {filename}")
    asset_dir = "assetslocation"
    asset_apk_xml = os.path.join(asset_dir, "apk", "textassets.xml")
    asset_ab_xml = os.path.join(asset_dir, "ab", "textassets.xml")
    name_input = os.path.splitext(filename)[0]
    ab_flag = False # flagging for next process, if True, meaning we need to use AB completion

    # Try to find asset in AB XML file
    asset, source = find_asset_in_xml(asset_ab_xml, name_input)

    if asset is not None:
        # Extract the Source element and print its text
        source_filename = os.path.basename(find_ab_filename(os.path.basename(source)))
        ab_flag = True
        print(f"[TYPE] OFFICIAL ASSETBUNDLE")
    else:
        # Try to find asset in APK XML file
        asset, source = find_asset_in_xml(asset_apk_xml, name_input)

        if asset is not None:
            # Extract the Source element and print its text
            source_filename = os.path.basename(source)
            print(f"[TYPE] APK ASSETBUNDLE")
        else:
            print(f"[FATAL ERROR] No asset found with Name '{name_input}'")
            return None

    return source_filename, ab_flag

def find_asset_in_xml(xml_path, name_input):
...
def process(source_filename, filename, ab_flag):
    if ab_flag == True:
        ab_type_process(source_filename, filename)
    else:
        apk_type_process(source_filename, filename)


def apk_type_process(source_filename, filename):
...
def ab_type_process(source_filename, filename):
...
def main():
    ...
    for filename in os.listdir(folder):
        source_filename, ab_flag = detect_assetbundle_type(filename)
        ...
        pyautogui.click()
        process(source_filename, filename, ab_flag)
...
if __name__ == '__main__':
    ...
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Elapsed time all process: {elapsed_time:.2f} seconds")
    print(f"Done !!")